...
async function main() {
    ...
    if (!platform.SupplyLimitModuleProxy) {
        throw new Error('SupplyLimitModuleProxy not found. Run register-gold-amoy.ts first.');
    }
    if (!platform.PhysicalReserveModuleProxy) {
        throw new Error('PhysicalReserveModuleProxy not found. Run register-gold-amoy.ts first.');
    }
    if (!platform.GoldAssetTypeRegisteredWithProxy) {
        throw new Error('GOLD type not registered with proxy modules. Run register-gold-amoy.ts first.');
    }
    ...
    // Must use the PROXY address — this is what AssetFactory binds to the asset's compliance contract
    const supplyLimitModule = await ethers.getContractAt('SupplyLimitModule', platform.SupplyLimitModuleProxy);
    ...
    // Attest Initial Reserve via PhysicalReserveModule proxy
    console.log('  Attesting initial physical reserve...');
    const physicalReserveModule = await ethers.getContractAt('PhysicalReserveModule', platform.PhysicalReserveModuleProxy);
    await (
    ...
    // Configure SupplyLimitModule proxy (bound by the Factory during creation)
    console.log('  Configuring SupplyLimitModule (proxy)...');
    ...
    // callModuleFunction must target the proxy address — same one AssetFactory used to bind the module
    await (await compliance.connect(deployer).callModuleFunction(setLimitData, platform.SupplyLimitModuleProxy, { gasPrice })).wait();
