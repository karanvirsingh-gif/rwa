 ...
 *
 * Upgradeability:
 *   Follows the UUPS pattern. Deploy via ERC1967Proxy pointing to this implementation.
 *   Only the holder of UPGRADER_ROLE (the admin / multisig) can upgrade the logic.
 *   Existing storage (assetId, paymentToken, totalReceived, roles) is preserved across upgrades.
 */
contract AssetTreasury is Initializable, UUPSUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    bytes32 public constant UPGRADER_ROLE   = keccak256("UPGRADER_ROLE");   // controls upgradeTo()
